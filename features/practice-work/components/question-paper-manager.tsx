...
export function QuestionPaperManager({ papers, questions, options }: { papers: PracticeSet[]; questions: BankQuestion[]; options: PracticeOptions }) {
  ...
  return <div className="space-y-6">
    ...
    <Card id="manual-paper"><CardHeader><CardTitle>Create Paper from Question Bank</CardTitle></CardHeader><CardContent className="space-y-4">
      <form action={(form) => finish(() => createQuestionPaperAction({ academicYearId: form.get("academicYearId"), paperType: form.get("paperType"), instructions: form.get("instructions") || undefined, questionIds: selectedQuestions }))} className="grid gap-4 md:grid-cols-3">
        ...
        <div className="md:col-span-3"><Input placeholder="Search Question Bank" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
        ...
        <Button disabled={pending || !selectedQuestions.length}>Create Editable Paper ({selectedQuestions.length})</Button>
      </form>
    </CardContent></Card>

    <Card id="combine-paper"><CardHeader><CardTitle>Combine 2 or 3 Existing Papers</CardTitle></CardHeader><CardContent><form action={(form) => finish(() => combineQuestionPapersAction({ sourcePaperIds: selectedPapers, paperType: form.get("paperType") }))} className="space-y-4">
