...
import { authorizeQuestionImportUploadAction, generateQuestionsAction, reviewGeneratedQuestionsAction, updateGeneratedQuestionAction } from "@/features/practice-work/actions/practice-work-actions";
import { saveUploadedQuestionFileAction } from "@/features/practice-work/actions/uploaded-question-file-actions";
...
export function AiGenerationManager({ templates, options, generationId, generationContext, questions, workspace = false }: Props) {
  ...
  const generateQuestions=async(event:FormEvent<HTMLFormElement>)=>{
    event.preventDefault();setMessage("Generating questions with Gemini…");setGenerationPending(true);const form=new FormData(event.currentTarget);
    try{
      const result=await generateQuestionsAction({boardId:form.get("boardId"),classId:form.get("classId"),subjectId:form.get("subjectId"),bookName:form.get("bookName"),chapter:form.get("chapter"),questionExamDate:form.get("questionExamDate"),sourceFullMarks:form.get("sourceFullMarks"),templateId:form.get("templateId"),questionCount:form.get("questionCount"),difficulty:form.get("difficulty"),specialInstructions:form.get("specialInstructions"),includeAnswers:true,includeExplanations:true,avoidDuplicates:true,keepLanguageSimple:true});
      setMessage(result.message);if(result.data)router.push(reviewHref(result.data.generationId));
    }catch{setMessage("Gemini generation is temporarily unavailable.")}finally{setGenerationPending(false)}
  };

  const importFile=async(event:FormEvent<HTMLFormElement>)=>{
    event.preventDefault();setImportPending(true);setMessage("Preparing question file upload…");
    const form=new FormData(event.currentTarget),file=form.get("file");
    try{
      if(!(file instanceof File)||!file.size)throw new Error("Select a question file.");
      const allowed=["application/pdf","image/jpeg","image/png"];
      if(!allowed.includes(file.type))throw new Error("Only PDF, JPG/JPEG and PNG files are supported.");
      const authorization=await authorizeQuestionImportUploadAction({filename:file.name,mimeType:file.type,byteSize:file.size});
      if(authorization.status==="error"||!authorization.data){setMessage(authorization.message);return}
      setMessage("Uploading original file to private storage…");
      const hash=await crypto.subtle.digest("SHA-256",await file.arrayBuffer()),clientSha256=Array.from(new Uint8Array(hash),byte=>byte.toString(16).padStart(2,"0")).join("");
      const upload=await createClient().storage.from("practice-work-private").uploadToSignedUrl(authorization.data.storagePath,authorization.data.uploadToken,file,{contentType:file.type});
      if(upload.error){setMessage(`Storage upload failed: ${upload.error.message}`);return}
      setMessage("Saving file details…");
      const result=await saveUploadedQuestionFileAction({sourceId:authorization.data.sourceId,storagePath:authorization.data.storagePath,originalFilename:file.name,declaredMimeType:file.type,declaredByteSize:file.size,clientSha256,boardId:form.get("boardId"),classId:form.get("classId"),subjectId:form.get("subjectId"),bookName:form.get("bookName"),chapter:form.get("chapter"),questionExamDate:form.get("questionExamDate"),sourceFullMarks:form.get("sourceFullMarks")});
      setMessage(result.message);
      if(result.status==="success"){event.currentTarget.reset();router.refresh()}
    }catch(error){setMessage(error instanceof Error?error.message:"The question file could not be uploaded.")}finally{setImportPending(false)}
  };

  return <div className="space-y-6">
    <Tabs defaultValue="ai">
      <TabsList><TabsTrigger value="ai">Generate with AI</TabsTrigger><TabsTrigger value="import">Import Question File</TabsTrigger></TabsList>
      <TabsContent value="ai"><Card><CardHeader><CardTitle>Generate with AI</CardTitle></CardHeader><CardContent>
        <form onSubmit={generateQuestions} className="grid gap-4 md:grid-cols-3">
      ...
      <TabsContent value="import"><Card><CardHeader><CardTitle>Import Question File</CardTitle></CardHeader><CardContent>
        <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-950"><strong>No AI extraction.</strong> The original PDF/JPG/PNG is stored exactly as uploaded together with the Board, Class, Subject, Book, Chapter, Exam Date and Full Marks entered below. It can then be assigned directly to students.</div>
        <form onSubmit={importFile} className="grid gap-4 md:grid-cols-3">
          {common("import")}
          <label className="text-sm font-medium md:col-span-2">Question File<Input name="file" type="file" required accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" /><span className="mt-1 block text-xs text-muted-foreground">Accepted: PDF, JPG/JPEG and PNG · Maximum 15 MB · Original file is preserved.</span></label>
          <div className="flex border-t pt-4 md:col-span-3"><Button type="submit" className="w-full sm:w-auto" disabled={importPending}>{importPending?"Saving File…":"Save Question File"}</Button></div>
        </form>
      </CardContent></Card></TabsContent>
    </Tabs>
    ...
    {generationId ? <section id="review" className="scroll-mt-24 space-y-4">
      <div><h3 className="text-xl font-semibold">Review AI-Generated Questions</h3><p className="text-sm text-muted-foreground">Only AI-generated drafts appear here. Uploaded files are not extracted.</p></div>
      ...
      <p className="text-sm text-muted-foreground">Proposed total marks: {questions.reduce((sum, question) => sum + question.suggestedMarks, 0)}{generationContext?.sourceFullMarks!==null&&generationContext?.sourceFullMarks!==undefined?` / Source Full Marks: ${generationContext.sourceFullMarks}`:""}</p>
      <div className="grid gap-4 lg:grid-cols-2">{questions.length ? questions.map((question) => <Card key={question.id}><CardContent className="space-y-3 p-5"><div className="flex items-start gap-3"><input aria-label={`Select ${question.questionText}`} type="checkbox" checked={selected.includes(question.id)} onChange={(event) => setSelected((value) => event.target.checked ? [...value, question.id] : value.filter((id) => id !== question.id))} /><div className="min-w-0 flex-1"><p className="font-semibold">{question.questionText}</p><p className="mt-2 text-sm">Answer: {String(question.correctAnswer)}</p><p className="text-sm text-muted-foreground">{question.explanation}</p><p className="mt-2 text-xs">{question.suggestedMarks} marks</p></div><Badge>{question.reviewStatus}</Badge></div>{question.duplicateWarning ? <p className="text-sm font-medium text-amber-700">Possible duplicate — review before approval.</p> : null}{question.reviewStatus === "pending" ? <Button type="button" size="sm" variant="outline" onClick={() => setEditing(question)}>Edit Draft</Button> : null}</CardContent></Card>) : <Card><CardContent className="p-8 text-center text-muted-foreground">No draft Questions were generated.</CardContent></Card>}</div>
    </section> : null}
