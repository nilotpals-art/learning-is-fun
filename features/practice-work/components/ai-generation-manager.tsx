...
export function AiGenerationManager({ templates, options, generationId, generationContext, questions, workspace = false }: Props) {
  ...
  const generateQuestions=(form:FormData)=>start(async()=>{setMessage("Generating questions with Gemini…");try{goToReview(await generateQuestionsAction({boardId:form.get("boardId"),classId:form.get("classId"),subjectId:form.get("subjectId"),bookName:form.get("bookName"),chapter:form.get("chapter"),questionExamDate:form.get("questionExamDate"),sourceFullMarks:form.get("sourceFullMarks"),templateId:form.get("templateId"),questionCount:form.get("questionCount"),difficulty:form.get("difficulty"),specialInstructions:form.get("specialInstructions"),includeAnswers:true,includeExplanations:true,avoidDuplicates:true,keepLanguageSimple:true}))}catch{setMessage("Gemini generation is temporarily unavailable.")}});
  ...
  return <div className="space-y-6">
    <Tabs defaultValue="ai">
      ...
      <TabsContent value="ai"><Card><CardHeader><CardTitle>Generate with AI</CardTitle></CardHeader><CardContent>
        <form action={generateQuestions} className="grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3"><Button disabled={pending || !templates.some((item) => item.isActive)}>{pending?"Generating…":"Generate Questions"}</Button></div>
        </form>
      ...
      <TabsContent value="import"><Card><CardHeader><CardTitle>Import File</CardTitle></CardHeader><CardContent>
        <form onSubmit={importFile} className="grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3"><Button type="submit" disabled={importPending}>Upload and Extract</Button></div>
        </form>
