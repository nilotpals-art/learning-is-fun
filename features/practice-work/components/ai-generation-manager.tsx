...
export function AiGenerationManager({ templates, options, generationId, generationContext, questions, workspace = false }: Props) {
  ...
  const [uploadState,setUploadState]=useState<"idle"|"authorizing"|"uploading"|"uploaded"|"validating"|"review"|"failed">("idle");
  ...
  return <div className="space-y-6">
    <Tabs defaultValue="ai">
      ...
      <TabsContent value="ai"><Card><CardHeader><CardTitle>Generate with AI</CardTitle></CardHeader><CardContent>
        <form action={(form) => start(async () => goToReview(await generateQuestionsAction({ boardId: form.get("boardId"), classId: form.get("classId"), subjectId: form.get("subjectId"), bookName: form.get("bookName"), chapter: form.get("chapter"), questionExamDate: form.get("questionExamDate"), sourceFullMarks: form.get("sourceFullMarks"), templateId: form.get("templateId"), questionCount: form.get("questionCount"), difficulty: form.get("difficulty"), specialInstructions: form.get("specialInstructions"), includeAnswers: true, includeExplanations: true, avoidDuplicates: true, keepLanguageSimple: true })))} className="grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3"><Button disabled={pending || !templates.some((item) => item.isActive)}>Generate Questions</Button></div>
        </form>
      ...
      <TabsContent value="import"><Card><CardHeader><CardTitle>Import File</CardTitle></CardHeader><CardContent>
        <form onSubmit={(event)=>start(()=>importFile(event))} className="grid gap-4 md:grid-cols-3">
          ...
          <div className="md:col-span-3"><Button disabled={pending||uploadState==="authorizing"||uploadState==="uploading"||uploadState==="validating"}>Upload and Extract</Button></div>
        </form>
        {uploadState!=="idle"?<p className="mt-3 text-sm font-medium" role="status">{{authorizing:"Authorizing upload…",uploading:"Uploading to private storage…",uploaded:"Upload complete",validating:"Validating document…",review:"Review required",failed:"Upload failed",idle:""}[uploadState]}</p>:null}
      </CardContent></Card></TabsContent>
    </Tabs>
