...
import type { UploadedQuestionFileAssignment } from "@/features/practice-work/services/uploaded-question-file-service";
import type { AttemptSession, PracticeAssignment, SubmitAttemptResult } from "@/features/practice-work/types/practice-work";

export function StudentPracticeManager({ assignments, fileAssignments }: { assignments: PracticeAssignment[]; fileAssignments: UploadedQuestionFileAssignment[] }) {
  ...
  })} className="space-y-5">{session.questions.map((question) => <fieldset key={question.id} className="rounded-xl border p-4"><legend className="px-2 font-semibold">{question.displayOrder}. {question.questionText}</legend>{question.options?.length ? <div className="mt-3 grid gap-2">{question.options.map((option) => <label key={`${question.id}-${option}`} className="flex gap-2"><input required name={question.id} type="radio" value={option}/><span>{option}</span></label>)}</div> : <Input className="mt-3" required name={question.id} aria-label={`Answer for question ${question.displayOrder}`}/>}<p className="mt-2 text-xs text-muted-foreground">{question.marks} marks</p></fieldset>)}<Button type="submit" disabled={pending}>Submit Practice</Button>{message && <p role="status" className="text-sm">{message}</p>}</form></CardContent></Card>;

  if (session && result) return <Card><CardHeader><CardTitle>Practice Review</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-2xl font-bold">{result.scoreObtained}/{result.maxMarks} · {result.percentage.toFixed(1)}%</p>{result.review.map((review) => <div key={review.questionId} className="rounded-xl border p-4"><div className="flex justify-between gap-3"><p className="font-semibold">{review.questionText}</p><Badge>{review.isCorrect ? "Correct" : "Needs practice"}</Badge></div><p className="mt-2 text-sm">Your answer: {String(review.studentAnswer ?? "Not answered")}</p><p className="text-sm">Correct answer: {String(review.correctAnswer)}</p>{review.explanation && <p className="mt-2 text-sm text-muted-foreground">{review.explanation}</p>}</div>)}{result.review.some((item) => !item.isCorrect) && <Button type="button" disabled={pending} onClick={() => startTransition(async () => { const response = await retryIncorrectAction({ attemptId: session.attemptId }); setMessage(response.message); if (response.status === "success" && response.data) { setSession(response.data); setResult(null); } })}>Retry Incorrect Questions</Button>}{message && <p role="status" className="text-sm">{message}</p>}</CardContent></Card>;

  return <div className="space-y-6">
    {fileAssignments.length ? <section className="space-y-3"><h3 className="text-lg font-semibold">Uploaded Question Files</h3><div className="grid gap-4 lg:grid-cols-2">{fileAssignments.map((assignment) => <Card key={assignment.id}><CardContent className="space-y-3 p-5"><div className="flex justify-between gap-3"><div><p className="font-semibold">{assignment.title}</p><p className="text-sm text-muted-foreground">{assignment.originalFilename}</p></div><Badge>{assignment.status}</Badge></div><p className="text-sm text-muted-foreground">Due {assignment.dueAt ? new Date(assignment.dueAt).toLocaleString() : "Not set"}</p><a className={buttonVariants({ variant: "default" })} href={`/practice-work/files/${assignment.id}`} target="_blank" rel="noreferrer">Open Question File</a></CardContent></Card>)}</div></section> : null}

    <section className="space-y-3"><h3 className="text-lg font-semibold">Interactive Practice</h3><div className="grid gap-4 lg:grid-cols-2">{assignments.length ? assignments.map((assignment) => <Card key={assignment.id}><CardContent className="space-y-3 p-5"><div className="flex justify-between gap-3"><p className="font-semibold">{assignment.practiceSetTitle}</p><Badge>{assignment.status.replaceAll("_", " ")}</Badge></div><p className="text-sm text-muted-foreground">Due {assignment.dueAt ? new Date(assignment.dueAt).toLocaleString() : "Not set"}</p><div className="flex flex-wrap gap-2"><a className={buttonVariants({ variant: "outline" })} href={`/practice-work/papers/${assignment.practiceSetId}/pdf`} target="_blank" rel="noreferrer">View PDF</a><Button type="button" disabled={pending || assignment.status === "closed"} onClick={() => start(assignment.id)}>{assignment.status === "completed" ? "Try Again" : "Start Practice"}</Button></div></CardContent></Card>) : <Card><CardContent className="p-8 text-center text-muted-foreground">No interactive Question Papers have been assigned yet.</CardContent></Card>}</div></section>
