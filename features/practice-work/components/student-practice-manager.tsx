...
export function StudentPracticeManager({ assignments }: { assignments: PracticeAssignment[] }) {
  ...
  return <div className="grid gap-4 lg:grid-cols-2">{assignments.length ? assignments.map((assignment) => <Card key={assignment.id}><CardContent className="space-y-3 p-5"><div className="flex justify-between gap-3"><p className="font-semibold">{assignment.practiceSetTitle}</p><Badge>{assignment.status.replaceAll("_", " ")}</Badge></div><p className="text-sm text-muted-foreground">Due {assignment.dueAt ? new Date(assignment.dueAt).toLocaleString() : "Not set"}</p><Button disabled={pending || assignment.status === "closed"} onClick={() => start(assignment.id)}>{assignment.status === "completed" ? "Try Again" : "Start Practice"}</Button></CardContent></Card>) : <Card><CardContent className="p-8 text-center text-muted-foreground">No Practice Work has been assigned yet.</CardContent></Card>}{message && <p role="status" className="text-sm">{message}</p>}</div>;
}
