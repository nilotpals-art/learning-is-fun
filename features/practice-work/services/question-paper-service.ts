...
export async function createQuestionPaper(profile: AuthProfile, input: {
  ...
  const { data: paper, error: paperError } = await s.from("practice_sets").insert({
    institute_id: instituteId,
    academic_year_id: input.academicYearId,
    board_id: first.board_id,
    class_id: first.class_id,
    title,
    description: input.paperType.trim().toUpperCase(),
    instructions: input.instructions?.trim() || "ANSWER ALL QUESTIONS.",
    answer_mode: "after_completion",
    allow_retry: false,
    marks_mode: "custom",
    target_total_marks: total,
    status: "draft",
    created_by: profile.id,
  }).select("id").single();
  if (paperError) throw paperError;

  const { error: snapshotError } = await s.from("practice_set_questions").insert(ordered.map((q, index) => ({
    institute_id: instituteId,
    practice_set_id: paper.id,
    ...
    marks: Number(q.suggested_marks ?? 1),
...
export async function combineQuestionPapers(profile: AuthProfile, input: { sourcePaperIds: string[]; paperType: string }) {
  ...
  const { data: paper, error: insertError } = await s.from("practice_sets").insert({
    institute_id: instituteId,
    academic_year_id: first.academic_year_id,
    board_id: first.board_id,
    class_id: first.class_id,
    title,
    description: (input.paperType || "COMBINED").trim().toUpperCase(),
  ...
  const { error: qError } = await s.from("practice_set_questions").insert(questions.map((q, index) => ({
    institute_id: instituteId,
    practice_set_id: paper.id,
...
export async function updateDraftQuestionPaper(profile: AuthProfile, input: {
  paperId: string;
  title: string;
  instructions?: string;
  questions: { id: string; questionText: string; marks: number; displayOrder: number }[];
}) {
  const s = await createClient();
  const instituteId = institute(profile);
