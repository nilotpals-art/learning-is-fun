...
} from "@/features/practice-work/services/gemini-question-generation-provider";
...
function errorDetails(error: unknown) {
  ...
  const message = rawMessage
    .replace(/\b(?:gsk_|sk-)[A-Za-z0-9_-]{12,}\b/g, "[REDACTED]")
    .slice(0, 500);
...
function providerErrorCode(httpStatus: number | undefined) {
  ...
  if (httpStatus === 500 || httpStatus === 502 || httpStatus === 503) {
    return "GROQ_GENERATION_SERVICE_UNAVAILABLE";
  }
...
export class GroqQuestionGenerationProvider {
  ...
  constructor(options?: {
    ...
    this.model = options?.model
      ?? process.env.GROQ_QUESTION_MODEL?.trim()
      ?? process.env.GROQ_MODEL?.trim()
      ?? DEFAULT_GROQ_QUESTION_MODEL;
  ...
  async generate(input: unknown): Promise<GeneratedQuestionOutput> {
    ...
    try {
      ...
      let value: unknown;
      try {
        value = JSON.parse(normalizeStructuredJson(text));
      } catch {
        throw new Error("GROQ_GENERATION_INVALID_JSON");
      }

      const parsed = aiQuestionOutputSchema.safeParse(value);
      if (!parsed.success) {
        console.warn("Groq question generation validation failed", {
          model: this.model,
