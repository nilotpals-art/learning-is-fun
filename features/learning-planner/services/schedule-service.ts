...
export async function listPlannerOptions(profile: AuthProfile): Promise<PlannerOptions> {
  ...
  return { academicYears: (years.data ?? []).map((year) => ({ id: year.id, label: year.name, isCurrent: year.is_current === true })), batches: (batches.data ?? []).map((batch) => ({ id: batch.id, label: batch.name, academicYearId: batch.academic_year_id, subjectId: batch.subject_id, subjectName: optionalOne(batch.subject)?.subject_name ?? null, branchId: batch.branch_id })), subjects: (subjects.data ?? []).map((subject) => ({ id: subject.id, label: subject.subject_name })), branches: map(branches.data) };
}
