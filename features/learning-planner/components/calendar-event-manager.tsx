...
export function CalendarEventManager({ events, options, holidays, providerAvailable }: { events: ScheduleEvent[]; options: PlannerOptions; holidays: PublicHoliday[]; providerAvailable: boolean }) {
  ...
  const filtered = useMemo(() => events.filter((event) => (!fromDate || event.eventDate >= fromDate) && (!toDate || event.eventDate <= toDate) && (!batchId || event.batchId === batchId) && (!eventType || event.scheduleType === eventType) && (!status || event.status === status)), [events, fromDate, toDate, batchId, eventType, status]);
