...
export function CalendarEventManager({ events, options }: { events: ScheduleEvent[]; options: PlannerOptions }) {
  ...
  const filtered = useMemo(() => events.filter((event) => (!fromDate || event.eventDate >= fromDate) && (!toDate || event.eventDate <= toDate) && (!batchId || event.batchId === batchId) && (!eventType || event.scheduleType === eventType) && (!status || event.status === status) && !(event.isProjected && event.scheduleType === "regular_class")), [events, fromDate, toDate, batchId, eventType, status]);
