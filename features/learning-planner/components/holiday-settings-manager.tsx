...
export function HolidaySettingsManager({settings,holidays,branches,providerAvailable}:{settings:HolidaySettings;holidays:PublicHoliday[];branches:PlannerOption[];providerAvailable:boolean}){
  ...
  return <div className="space-y-6">
    <Card><CardHeader><CardTitle>India Holiday Settings</CardTitle></CardHeader><CardContent className="space-y-4">
      <p className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Your institute is configured for West Bengal. National holidays and West Bengal State holidays can both be shown in the planner.</p>
      <form action={()=>run(()=>saveHolidaySettingsAction({stateCode,showNationalHolidays:showNational,showStateHolidays:showState,portalThemeEnabled}))} className="grid gap-4 md:grid-cols-2">
