...
export function HolidaySettingsManager({settings,holidays,branches,providerAvailable}:{settings:HolidaySettings;holidays:PublicHoliday[];branches:PlannerOption[];providerAvailable:boolean}){
  ...
  return <div className="space-y-6">
    <Card><CardHeader><CardTitle>India Holiday Settings</CardTitle></CardHeader><CardContent className="space-y-4">
      {!stateCode?<p className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">Select your State / Union Territory and save once to load local/state holidays.</p>:null}
      <form action={()=>run(()=>saveHolidaySettingsAction({stateCode:stateCode||null,showNationalHolidays:showNational,showStateHolidays:showState}))} className="grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-medium">State / Union Territory<select value={stateCode} onChange={event=>{const value=event.target.value;setStateCode(value);if(value)setShowState(true);}} className="mt-1 h-10 w-full rounded-xl border bg-card px-3"><option value="">Select State / UT</option>{STATES.map(([code,name])=><option key={code} value={code}>{name}</option>)}</select></label>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={showNational} onChange={event=>setShowNational(event.target.checked)}/>Show India national holidays</label>
