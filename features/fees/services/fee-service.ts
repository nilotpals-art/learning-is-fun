...
export async function listFeeDues(profile: AuthProfile, studentId?: string | string[]): Promise<FeeDue[]> {
  ...
  return (data ?? []).map((row) => {
    ...
    return { id: row.id, studentId: row.student_id, studentName: student?.name ?? "STUDENT", admissionNo: student?.admission_no ?? null, academicYearId: row.academic_year_id, academicYearName: year?.name ?? "ACADEMIC YEAR", feeHeadName: head?.name ?? "FEE", dueDate: row.due_date, netAmount: Number(row.net_amount), outstanding: Math.max(Number(row.net_amount) - paid, 0), status: row.status, scheduleType: (structureItem?.schedule_type ?? null) as FeeDue["scheduleType"] };
  });
}

export async function listFeePayments(profile: AuthProfile, studentId?: string | string[]): Promise<FeePayment[]> {
  const instituteId = scope(profile); const supabase = await createClient();
  let query = supabase.from("fee_payments").select("id,student_id,academic_year_id,payment_date,amount,receipt_no,reference_no,remarks,status,reversal_reason,student:students(name),year:academic_years(name),mode:payment_modes(name)").eq("institute_id", instituteId).order("payment_date", { ascending: false });
  if (Array.isArray(studentId)) query = query.in("student_id", studentId);
  else if (studentId) query = query.eq("student_id", studentId);
  const { data, error } = await query; check(error);
...
export async function getFeeSettings(profile: AuthProfile): Promise<FeeSettings> {
  ...
  return {
    defaultMonthlyDueDay: data.default_monthly_due_day ?? 15,
    whatsappFeeRemindersEnabled: data.whatsapp_fee_reminders_enabled,
    reminderAfterDueDays: data.reminder_after_due_days,
    repeatEveryDays: data.repeat_every_days,
    maxRemindersPerDue: data.max_reminders_per_due,
    whatsappPaymentConfirmationsEnabled: data.whatsapp_payment_confirmations_enabled,
