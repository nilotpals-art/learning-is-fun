...
export async function sendVerifiedFeeReceipt(input:{paymentId:string;channel:"whatsapp"|"email"|"both"}){
 ...
 const {data:p,error}=await sb.from("fee_payments").select("id,student_id,payment_date,amount,receipt_no,reference_no,status,payment_mode_id,student:students!fee_payments_student_id_fkey(name,admission_no),mode:payment_modes!fee_payments_payment_mode_id_fkey(name)").eq("institute_id",profile.instituteId).eq("id",input.paymentId).maybeSingle();
 if(error||!p||p.status!=="posted")return {status:"error" as const,message:"Posted payment not found."};
 ...
 const {data:allocs}=await sb.from("fee_payment_allocations").select("amount,due:student_fee_dues!fee_payment_allocations_due_id_fkey(due_date,assignment:student_fee_assignments!student_fee_dues_assignment_fkey(head:fee_heads!student_fee_assignments_fee_head_id_fkey(name),structure_item:class_fee_structure_items!student_fee_assignments_class_fee_structure_item_id_fkey(schedule_type)))").eq("payment_id",p.id);
