...
const mobileSchema = z.string().trim().regex(/^[6-9][0-9]{9}$/, "Enter a valid 10-digit Indian contact number.");
...
export async function deleteEnrollmentLinkAction(inviteIdInput: unknown) {
  const inviteId = z.string().uuid().safeParse(inviteIdInput);
  if (!inviteId.success) return { status: "error" as const, message: "Invalid enrollment link." };

  const profile = await requireRole(DASHBOARD_ROLES);
  if (!profile.instituteId) return { status: "error" as const, message: "Institute is unavailable." };

  try {
    await deleteEnrollmentInvite(profile.instituteId, inviteId.data);
    revalidatePath("/students/enrollment-links");
    return { status: "success" as const, message: "Enrollment link deleted." };
  } catch {
    return { status: "error" as const, message: "Could not delete the enrollment link." };
  }
}
...
export async function submitEnrollmentAction(token: string, input: unknown) {
  const schema = z.object({
    ...
    dateOfBirth: z.string().refine(
      (value) => !Number.isNaN(Date.parse(`${value}T00:00:00`)) && new Date(`${value}T00:00:00`) <= new Date(),
    ...
    schoolBoardId: z.string().uuid("Select a School Board."),
    address: z.string().trim().optional().default(""),
    parentName: z.string().trim().min(1),
    parentEmail: optionalEmailSchema,
    parentNoEmail: z.boolean(),
    parentRequest: z.string().trim().max(1000, "Comments / Request is too long.").optional().default(""),
  ...
  try {
    ...
    if (message.includes("PARENT_ENROLLMENT_BOARD_INVALID")) return { status: "error" as const, message: "Select a valid School Board." };
