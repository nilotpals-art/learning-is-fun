...
interface BatchNameRow { id: string; name: string }
...
function toDashboardEvent(event: ScheduleEvent): StudentDashboardEvent {
  return { id: event.id, title: event.title, subjectName: event.subjectName, batchName: event.batchName, eventDate: event.eventDate, startTime: event.startTime, endTime: event.endTime, scheduleType: event.scheduleType, status: event.status };
}

async function getAssignmentWindows(profile: AuthProfile, studentId: string, fromDate: string, toDate: string): Promise<Map<string, Array<{ fromDate: string; toDate: string }>>> {
  if (!profile.instituteId) throw new Error("STUDENT_DASHBOARD_UNAUTHORIZED");
  const supabase = await createClient();
  const { data, error } = await supabase.from("student_assignments").select("batch_id,effective_from,effective_to").eq("student_id", studentId).eq("institute_id", profile.instituteId).lte("effective_from", toDate).or(`effective_to.is.null,effective_to.gte.${fromDate}`);
  if (error) throw error;
  const windows = new Map<string, Array<{ fromDate: string; toDate: string }>>();
  for (const assignment of (data ?? []) as AssignmentWindowRow[]) {
    const batchWindows = windows.get(assignment.batch_id) ?? [];
    batchWindows.push({ fromDate: assignment.effective_from > fromDate ? assignment.effective_from : fromDate, toDate: assignment.effective_to && assignment.effective_to < toDate ? assignment.effective_to : toDate });
    windows.set(assignment.batch_id, batchWindows);
  }
  return windows;
}

async function getStudentCalendar(profile: AuthProfile, studentId: string, fromDate: string, toDate: string): Promise<ScheduleEvent[]> {
  ...
  return events.filter((event) => event.scheduleType === "holiday" || (event.batchId !== null && windows.get(event.batchId)?.some((window) => event.eventDate >= window.fromDate && event.eventDate <= window.toDate)));
}

async function getIdentity(profile: AuthProfile): Promise<StudentDashboardIdentity> {
  if (!profile.instituteId) throw new Error("STUDENT_DASHBOARD_UNAUTHORIZED");
  const supabase = await createClient();
  const { data: student, error: studentError } = await supabase.from("students").select("id,name").eq("profile_id", profile.id).eq("institute_id", profile.instituteId).single();
  if (studentError || !student) throw new Error("STUDENT_DASHBOARD_IDENTITY_NOT_FOUND");

  const { data: assignment, error: assignmentError } = await supabase
    .from("student_assignments")
    .select("academic_year_id,batch_id,academic_year:academic_years!student_assignments_academic_year_fkey(name),board:boards!student_assignments_board_fkey(name),academic_class:academic_classes!student_assignments_class_fkey(class_name)")
  ...
  if (current?.batch_id) {
    const { data: batch, error: batchError } = await supabase.from("batches").select("id,name").eq("id", current.batch_id).eq("institute_id", profile.instituteId).maybeSingle();
    if (batchError) throw batchError;
    batchName = (batch as BatchNameRow | null)?.name ?? null;
  }
...
async function getAttendance(profile: AuthProfile, student: StudentDashboardIdentity): Promise<AttendanceTotals | null> {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("student_attendance_summary", { p_institute_id: profile.instituteId, p_student_id: student.id, p_academic_year_id: student.academicYearId, p_date_from: null, p_date_to: null });
...
async function getPractice(profile: AuthProfile, student: StudentDashboardIdentity): Promise<{ summary: StudentPracticeSummary; progress: StudentPracticeProgress }> {
  ...
  const latestByAssignment = new Map<string, AttemptRow>(); attempts.forEach((attempt) => { if (!latestByAssignment.has(attempt.practice_assignment_id)) latestByAssignment.set(attempt.practice_assignment_id, attempt); });
  ...
  const retryPairs = attempts.filter((item) => item.attempt_no > 1 && item.percentage !== null).map((retry) => { const first = attempts.find((item) => item.practice_assignment_id === retry.practice_assignment_id && item.attempt_no === 1 && item.percentage !== null); return first ? Number(retry.percentage) - Number(first.percentage) : null; }).filter((value): value is number => value !== null);
  return { summary: { pending: assignments.filter((item) => item.status === "assigned").length, inProgress: assignments.filter((item) => item.status === "in_progress").length, completed: assignments.filter((item) => item.status === "completed").length, dueSoon: assignments.filter((item) => item.due_at && item.status !== "completed" && new Date(item.due_at) >= now && new Date(item.due_at) <= soon).length, overdue: assignments.filter((item) => item.due_at && item.status !== "completed" && new Date(item.due_at) < now).length, actionableItem: actionable ? { assignmentId: actionable.id, title: one(actionable.set)?.title ?? "Practice Work", skill: one(actionable.set)?.skill ?? null, topic: one(actionable.set)?.topic ?? null, status: actionable.status, dueAt: actionable.due_at, latestPercentage: actionableAttempt?.percentage == null ? null : Number(actionableAttempt.percentage), scoreObtained: actionableAttempt?.score_obtained == null ? null : Number(actionableAttempt.score_obtained), maxMarks: actionableAttempt ? Number(actionableAttempt.max_marks) : null } : null }, progress: { submittedAttempts: attempts.length, completedSets: new Set(attempts.map((item) => item.practice_assignment_id)).size, averagePercentage: percentages.length ? percentages.reduce((sum, value) => sum + value, 0) / percentages.length : null, latestPercentage: percentages[0] ?? null, firstAttemptPercentage: firstAttempts.length ? firstAttempts.reduce((sum, value) => sum + value, 0) / firstAttempts.length : null, retryImprovement: retryPairs.length ? retryPairs.reduce((sum, value) => sum + value, 0) / retryPairs.length : null } };
}

export async function getStudentDashboardData(profile: AuthProfile): Promise<StudentDashboardData> {
  const student = await getIdentity(profile); const today = new Date(); const upcomingEnd = new Date(today); upcomingEnd.setDate(upcomingEnd.getDate() + 14);
  ...
  const events = eventsResult.map(toDashboardEvent); const todayValue = dateValue(today);
  ...
  const learningEvents = events.filter((event) => event.scheduleType !== "holiday"); const todaysEvents = learningEvents.filter((event) => event.eventDate === todayValue); const nextEvent = todaysEvents.find((event) => event.status !== "cancelled" && (!event.startTime || event.startTime >= today.toTimeString().slice(0, 5))) ?? null;
  return { student, quote, todaysEvents, holidays, nextEvent, practice: practiceResult.summary, attendance: attendanceResult, progress: practiceResult.progress, upcomingEvents: learningEvents.filter((event) => event.eventDate > todayValue).slice(0, 6), notifications: notificationsResult.slice(0, 5).map((notification) => ({ recipientId: notification.recipientId, title: notification.title, message: notification.message, priority: notification.priority, readAt: notification.readAt, createdAt: notification.createdAt })), unreadNotifications: notificationsResult.filter((notification) => !notification.readAt).length, recentResults: results.slice(0, 5) };
}

export async function getStudentSchedule(profile: AuthProfile): Promise<StudentDashboardEvent[]> { const student = await getIdentity(profile); const from = new Date(); const to = new Date(from); to.setDate(to.getDate() + 30); return (await getStudentCalendar(profile, student.id, dateValue(from), dateValue(to))).filter((event) => event.scheduleType !== "holiday").map(toDashboardEvent); }
...
export async function getStudentAttendance(profile: AuthProfile): Promise<AttendanceTotals | null> { return getAttendance(profile, await getIdentity(profile)); }
