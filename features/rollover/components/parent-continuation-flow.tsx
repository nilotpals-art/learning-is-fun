...
type FlowStep = "response" | "batch" | "review" | "done";
...
export function ParentContinuationFlow({ detail, batches }: {
  ...
  function saveSimple(next: FlowResponse) {
    startTransition(async () => {
      const result = await saveParentRolloverResponse({ requestId: detail.requestId, parentResponse: next, selectedBatchId: null, notes: notes || null });
  ...
  function confirm() {
    ...
    startTransition(async () => {
      const saveResult = await saveParentRolloverResponse({ requestId: detail.requestId, parentResponse: "continuing", selectedBatchId, notes: notes || null });
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><Toaster /><Card><CardContent className="space-y-6 p-6">
    <StepIndicator current={step === "response" ? 1 : step === "batch" ? 2 : 3} />
    ...
    {step === "review" ? <div className="space-y-4"><h3 className="text-sm font-semibold">Review and confirm</h3>
      ...
      <div className="flex justify-between"><Button variant="outline" onClick={() => setStep("batch")} disabled={pending}><ArrowLeft />Back</Button><Button onClick={confirm} disabled={pending}>{pending ? "Confirming…" : "Confirm Continuation"}</Button></div>
    </div> : null}
  </CardContent></Card></div>;
}