...
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.class_fee_structure_items i
    WHERE i.schedule_type='monthly'
      AND NOT EXISTS (SELECT 1 FROM public.class_fee_structure_installments ci WHERE ci.class_fee_structure_item_id=i.id)
      AND EXISTS (SELECT 1 FROM public.student_fee_assignments a WHERE a.class_fee_structure_item_id=i.id)
  ) THEN
    RAISE EXCEPTION 'Cannot roll back monthly recurring semantics after monthly assignments exist.';
  END IF;
END $$;
...
SELECT i.institute_id,i.id,g.n,
       (ay.start_date+(g.n-1)*interval '1 month')::date,
       CASE WHEN g.n=12 THEN i.amount-(trunc(i.amount/12,2)*11) ELSE trunc(i.amount/12,2) END
FROM public.class_fee_structure_items i
JOIN public.class_fee_structures s ON s.id=i.class_fee_structure_id
JOIN public.academic_years ay ON ay.id=s.academic_year_id
CROSS JOIN generate_series(1,12) AS g(n)
WHERE i.schedule_type='monthly'
  AND NOT EXISTS (SELECT 1 FROM public.class_fee_structure_installments ci WHERE ci.class_fee_structure_item_id=i.id);
CREATE OR REPLACE FUNCTION public.save_class_fee_structure(p_structure_id uuid,p_academic_year_id uuid,p_class_id uuid,p_name text,p_is_active boolean,p_items jsonb)
...
ALTER TABLE public.fee_settings
  DROP CONSTRAINT fee_settings_monthly_due_day_check,
  DROP COLUMN default_monthly_due_day;
