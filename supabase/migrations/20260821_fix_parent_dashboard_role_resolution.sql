...
declare v_profile public.profiles%rowtype; v_role text; v_results jsonb := '[]'::jsonb; v_row record;
begin
  select p.*, coalesce(nullif(btrim(p.role),''), r.name)
  into v_profile, v_role
  from public.profiles p
  left join public.roles r on r.id=p.role_id
  where p.id=auth.uid() and p.is_active is true;
  if v_profile.id is null or v_profile.institute_id is null or v_role <> 'Parent' then raise exception 'PARENT_UNAUTHORIZED'; end if;
  for v_row in
...
declare v_institute_id uuid; v_role text; v_results jsonb;
begin
  ...
  where p.id=auth.uid() and p.is_active is true;
  if v_institute_id is null or v_role <> 'Parent' or not public.fee_parent_can_view_student(v_institute_id,p_student_id) then raise exception 'PARENT_UNAUTHORIZED'; end if;
