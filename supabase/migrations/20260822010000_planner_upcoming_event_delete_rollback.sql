...
begin
  ...
  if v_event.event_date < current_date then raise exception 'PLANNER_EVENT_PAST'; end if;
  ...
  from public.schedule_events where institute_id=v_profile.institute_id and (id=v_root_id or original_event_id=v_root_id);

  if exists(select 1 from public.exam_result_sets where schedule_event_id=any(v_ids))
     or exists(select 1 from public.practice_assignments where schedule_event_id=any(v_ids))
     or exists(select 1 from public.practice_sets where schedule_event_id=any(v_ids)) then
    raise exception 'PLANNER_EVENT_HAS_DEPENDENCIES';
  end if;
  ...
  select array_agg(id) into v_notification_ids from public.notifications
  ...
  if coalesce(cardinality(v_notification_ids),0)>0 then
    ...
    if to_regclass('public.admin_notification_campaigns') is not null and exists(select 1 from public.admin_notification_campaigns where notification_id=any(v_notification_ids)) then
      raise exception 'PLANNER_EVENT_HAS_DEPENDENCIES';
    end if;
  ...
  return jsonb_build_object('deleted',coalesce(cardinality(v_ids),0),'restoredRecurringClass',v_event.class_schedule_id is not null);
end;
$function$;
...
create or replace function public.delete_planner_history_entry(p_change_id uuid)
returns void
language plpgsql
security definer
set search_path to ''
...
begin
  ...
  if v_role is null then v_role:=nullif(btrim(v_profile.role),''); end if;
  if v_role not in ('admin','Super Admin','Institute Admin') then raise exception 'PLANNER_DELETE_UNAUTHORIZED'; end if;
  delete from public.schedule_changes sc using public.schedule_events se
  where sc.id=p_change_id and sc.schedule_event_id=se.id and se.institute_id=v_profile.institute_id;
  if not found then raise exception 'PLANNER_HISTORY_NOT_FOUND'; end if;
end;
$function$;
revoke all on function public.delete_planner_history_entry(uuid) from public,anon;
grant execute on function public.delete_planner_history_entry(uuid) to authenticated;
