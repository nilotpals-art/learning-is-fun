...
-- Fix: grant update privilege needed for UI to toggle Batch active state
GRANT UPDATE ON public.batches TO authenticated;
...
BEGIN
  ...
  -- Ensure the target batch belongs to the caller's institute
  IF NOT EXISTS (
    SELECT 1
    FROM public.batches b
    WHERE b.id = p_batch_id
      AND b.institute_id = v_profile.institute_id
  ) THEN
    RAISE EXCEPTION 'BATCH_NOT_FOUND_OR_OUT_OF_SCOPE';
  END IF;
  ...
  -- Schedule changes for events (scoped by event)
