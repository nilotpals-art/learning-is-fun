...
ALTER TABLE public.students
  ADD COLUMN IF NOT EXISTS school_board_id uuid;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'students_school_board_institute_fkey'
...
CREATE INDEX IF NOT EXISTS students_school_board_idx
  ON public.students (school_board_id);

DROP FUNCTION IF EXISTS public.submit_parent_enrollment(
  text,text,text,text,date,text,text,text,text,text,text,text,text,boolean
);
DROP FUNCTION IF EXISTS public.submit_parent_enrollment(
  text,text,text,text,date,text,text,text,text,text,text,boolean
);
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE b.id = p_school_board_id
      AND b.institute_id = v_invite.institute_id
  ) THEN
    RAISE EXCEPTION 'PARENT_ENROLLMENT_BOARD_INVALID';
  END IF;
  ...
  IF v_student_email IS NOT NULL
     ...
     AND v_student_email = v_parent_email THEN
  ...
  SET school_name = NULLIF(upper(btrim(coalesce(p_school_name, ''))), ''),
      school_board_id = p_school_board_id,
      updated_at = now()
  ...
  PERFORM public.apply_class_fee_structure(
    v_student_id,
    v_invite.fee_structure_id,
    v_fee_items
  );
...
REVOKE ALL ON FUNCTION public.submit_parent_enrollment(
  text,text,text,date,text,text,text,uuid,text,text,text,text,boolean
) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.submit_parent_enrollment(
  text,text,text,date,text,text,text,uuid,text,text,text,text,boolean
) TO service_role;
