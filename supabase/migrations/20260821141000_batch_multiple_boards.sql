create table public.batch_boards (
  batch_id uuid not null,
  institute_id uuid not null,
  board_id uuid not null,
  class_id uuid not null,
  created_at timestamptz not null default now(),
  constraint batch_boards_pkey primary key (batch_id, board_id),
  constraint batch_boards_assignment_compatibility_key unique (batch_id, institute_id, board_id, class_id),
  constraint batch_boards_batch_fkey foreign key (batch_id, institute_id) references public.batches(id, institute_id) on delete cascade,
  constraint batch_boards_board_fkey foreign key (board_id, institute_id) references public.boards(id, institute_id) on delete restrict
);
create index batch_boards_institute_board_idx on public.batch_boards(institute_id, board_id);
insert into public.batch_boards(batch_id, institute_id, board_id, class_id)
select id, institute_id, board_id, class_id from public.batches
where board_id is not null and class_id is not null
on conflict (batch_id, board_id) do nothing;
alter table public.batch_boards enable row level security;
create policy batch_boards_admin_select on public.batch_boards for select to authenticated
using (exists (select 1 from public.batches b where b.id=batch_boards.batch_id and b.institute_id=batch_boards.institute_id and public.learning_planner_admin_scope(b.institute_id,b.branch_id)));
grant select on public.batch_boards to authenticated;
revoke insert, update, delete on public.batch_boards from authenticated, anon;
alter table public.student_assignments drop constraint student_assignments_batch_compatibility_fkey;
alter table public.student_assignments add constraint student_assignments_batch_compatibility_fkey foreign key (batch_id,institute_id,board_id,class_id) references public.batch_boards(batch_id,institute_id,board_id,class_id) on delete restrict;
...
declare v_profile public.profiles%rowtype;v_batch public.batches%rowtype;v_group jsonb;v_day text;v_schedule_id uuid;v_conflict record;v_conflicts jsonb:='[]'::jsonb;v_created jsonb:='[]'::jsonb;v_board_ids uuid[];v_primary_board_id uuid;
begin
 ...
 select array_agg(x.board_id order by x.board_id) into v_board_ids from (select distinct value::uuid board_id from jsonb_array_elements_text(case when jsonb_typeof(p_input->'boardIds')='array' then p_input->'boardIds' when nullif(p_input->>'boardId','') is not null then jsonb_build_array(p_input->>'boardId') else '[]'::jsonb end)) x;
 ...
 if exists(select 1 from public.batches b where b.institute_id=v_profile.institute_id and b.branch_id is not distinct from nullif(p_input->>'branchId','')::uuid and b.academic_year_id=(p_input->>'academicYearId')::uuid and b.board_id=v_primary_board_id and b.class_id=(p_input->>'classId')::uuid and b.subject_id=(p_input->>'subjectId')::uuid and upper(btrim(b.name))=upper(btrim(p_input->>'name')) and b.is_active is true) then raise exception 'BATCH_DUPLICATE_IDENTITY';end if;
 ...
 insert into public.batches(institute_id,branch_id,academic_year_id,board_id,class_id,subject_id,name,is_active) values(v_profile.institute_id,nullif(p_input->>'branchId','')::uuid,(p_input->>'academicYearId')::uuid,v_primary_board_id,(p_input->>'classId')::uuid,(p_input->>'subjectId')::uuid,upper(btrim(p_input->>'name')),true) returning * into v_batch;
 insert into public.batch_boards(batch_id,institute_id,board_id,class_id) select v_batch.id,v_batch.institute_id,board_id,v_batch.class_id from unnest(v_board_ids) board_id;
 for v_group in select value from jsonb_array_elements(p_input->'schedules') loop
  for v_day in select jsonb_array_elements_text(v_group->'days') loop
...
begin
 ...
 if not exists(select 1 from public.batches b join public.batch_boards bb on bb.batch_id=b.id and bb.institute_id=b.institute_id and bb.class_id=b.class_id where b.id=p_batch_id and b.institute_id=v_institute_id and bb.board_id=p_board_id and b.class_id=p_class_id and b.is_active is true) then raise exception 'STUDENT_ASSIGNMENT_BATCH_INCOMPATIBLE';end if;
...
exception when exclusion_violation then raise exception 'STUDENT_ASSIGNMENT_OVERLAP';when unique_violation then raise exception 'STUDENT_ASSIGNMENT_CURRENT_CONFLICT';end;$function$;
...
begin select * into v_request from public.student_rollover_requests where id=p_request_id;if v_request.id is null then return false;end if;select count(*) into v_existing_count from public.batches b join public.batch_boards bb on bb.batch_id=b.id and bb.institute_id=b.institute_id and bb.class_id=b.class_id where b.id=p_batch_id and b.institute_id=v_request.institute_id and b.academic_year_id=v_request.target_academic_year_id and bb.board_id=v_request.proposed_board_id and b.class_id=v_request.proposed_class_id and b.is_active is true;if v_existing_count=0 then return false;end if;if p_require_availability then select public.rollover_seat_availability(p_batch_id,v_request.target_academic_year_id,v_request.id) into v_available;if v_available is not null and v_available<1 then return false;end if;end if;return true;end;$function$;
...
begin select * into v_request from public.student_rollover_requests where id=p_request_id;if v_request.id is null then raise exception 'ROLLOVER_REQUEST_NOT_FOUND';end if;select * into v_profile from public.profiles p where p.id=auth.uid() and p.is_active is true;select coalesce(nullif(btrim(p.role),''),r.name) into v_role from public.profiles p left join public.roles r on r.id=p.role_id where p.id=auth.uid();if v_profile.id is null then raise exception 'ROLLOVER_UNAUTHORIZED';end if;if v_role in ('admin','Super Admin','Institute Admin') then if v_profile.institute_id<>v_request.institute_id then raise exception 'ROLLOVER_UNAUTHORIZED';end if;elsif v_role='Parent' then if not exists(select 1 from public.parents pa join public.student_parent_links spl on spl.parent_id=pa.id and spl.institute_id=pa.institute_id where pa.profile_id=v_profile.id and pa.institute_id=v_request.institute_id and pa.is_active is true and spl.student_id=v_request.student_id) then raise exception 'ROLLOVER_UNAUTHORIZED';end if;else raise exception 'ROLLOVER_UNAUTHORIZED';end if;for v_row in select b.id,b.name,b.branch_id,b.subject_id,b.capacity,br.name branch_name,s.subject_name,(select count(*) from public.student_assignments sa where sa.batch_id=b.id and sa.academic_year_id=v_request.target_academic_year_id and sa.status='Current') assigned,(select count(*) from public.student_rollover_requests rr where rr.selected_batch_id=b.id and rr.target_academic_year_id=v_request.target_academic_year_id and rr.parent_locked_at is not null and rr.admin_status in ('ready','approved') and rr.id<>v_request.id) reserved from public.batches b join public.batch_boards bb on bb.batch_id=b.id and bb.institute_id=b.institute_id and bb.class_id=b.class_id left join public.branches br on br.id=b.branch_id and br.institute_id=b.institute_id left join public.subjects s on s.id=b.subject_id and s.institute_id=b.institute_id where b.institute_id=v_request.institute_id and b.academic_year_id=v_request.target_academic_year_id and bb.board_id=v_request.proposed_board_id and b.class_id=v_request.proposed_class_id and b.is_active is true order by b.name loop v_available:=case when v_row.capacity is null then null else v_row.capacity-v_row.assigned-v_row.reserved end;v_results:=v_results||jsonb_build_object('batch_id',v_row.id,'batch_name',v_row.name,'branch_name',v_row.branch_name,'subject_name',v_row.subject_name,'capacity',v_row.capacity,'assigned',v_row.assigned,'reserved',v_row.reserved,'available',v_available);end loop;return v_results;end;$function$;
