...
export default async function HistoryPage(){const profile=await requireRole(DASHBOARD_ROLES);if(!profile.instituteId)redirect("/unauthorized");return <PlannerShell title="Schedule History" description="Review the immutable lifecycle history of Schedule Events."><HistoryList changes={await listScheduleChanges(profile)}/></PlannerShell>}
