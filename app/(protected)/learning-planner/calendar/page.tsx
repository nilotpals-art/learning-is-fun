...
export default async function CalendarPage() {
  ...
  const [events, options] = await Promise.all([listCalendarReadModel(profile, fromDate, toDate), listPlannerOptions(profile)]);
  return <PlannerShell title="Academic Calendar" description="Browse class changes, tests, exams, and meetings. Holidays are managed separately in the Holidays tab."><CalendarEventManager events={events} options={options} /></PlannerShell>;
}
